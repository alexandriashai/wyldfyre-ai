...
tmux send-keys -t "$SESSION_NAME:wyld" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:wyld" "echo '🔥 Starting Wyld (Supervisor Agent)...'" C-m
tmux send-keys -t "$SESSION_NAME:wyld" "python -m services.supervisor.src.supervisor.agent 2>&1 | tee logs/wyld.log" C-m

# Code Agent - Window 1
tmux new-window -t "$SESSION_NAME" -n "code"
tmux send-keys -t "$SESSION_NAME:code" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:code" "echo 'Starting Code Agent...'" C-m
tmux send-keys -t "$SESSION_NAME:code" "python -m services.agents.code_agent.src.code_agent.agent 2>&1 | tee logs/code_agent.log" C-m

# Data Agent - Window 2
tmux new-window -t "$SESSION_NAME" -n "data"
tmux send-keys -t "$SESSION_NAME:data" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:data" "echo 'Starting Data Agent...'" C-m
tmux send-keys -t "$SESSION_NAME:data" "python -m services.agents.data_agent.src.data_agent.agent 2>&1 | tee logs/data_agent.log" C-m

# Infrastructure Agent - Window 3
tmux new-window -t "$SESSION_NAME" -n "infra"
tmux send-keys -t "$SESSION_NAME:infra" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:infra" "echo 'Starting Infrastructure Agent...'" C-m
tmux send-keys -t "$SESSION_NAME:infra" "python -m services.agents.infra_agent.src.infra_agent.agent 2>&1 | tee logs/infra_agent.log" C-m

# Research Agent - Window 4
tmux new-window -t "$SESSION_NAME" -n "research"
tmux send-keys -t "$SESSION_NAME:research" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:research" "echo 'Starting Research Agent...'" C-m
tmux send-keys -t "$SESSION_NAME:research" "python -m services.agents.research_agent.src.research_agent.agent 2>&1 | tee logs/research_agent.log" C-m

# QA Agent - Window 5
tmux new-window -t "$SESSION_NAME" -n "qa"
tmux send-keys -t "$SESSION_NAME:qa" "cd $PROJECT_ROOT && source .venv/bin/activate 2>/dev/null || true" C-m
tmux send-keys -t "$SESSION_NAME:qa" "echo 'Starting QA Agent...'" C-m
tmux send-keys -t "$SESSION_NAME:qa" "python -m services.agents.qa_agent.src.qa_agent.agent 2>&1 | tee logs/qa_agent.log" C-m

# Logs window - Window 6
tmux new-window -t "$SESSION_NAME" -n "logs"
tmux send-keys -t "$SESSION_NAME:logs" "cd $PROJECT_ROOT/logs && tail -f *.log 2>/dev/null || echo 'Waiting for logs...'" C-m
