...
All source code lives in `/home/wyld-core/` as a single git repository.

```
/home/wyld-core/                    # GIT REPO - All source code lives here
├── .git/                           # Single repo for everything
├── .env                            # Secrets (gitignored, 600 perms)
├── .env.example                    # Template for secrets
├── .gitignore                      # Excludes secrets, logs, .venv, node_modules
├── Makefile                        # Developer commands
├── docker-compose.yml              # Production compose
├── docker-compose.dev.yml          # Dev overrides (hot reload)
│
├── packages/                       # Shared Python libraries (installable)
│   ├── core/                       # ai_core - config, logging, metrics
│   ├── messaging/                  # ai_messaging - Redis pub/sub
│   ├── memory/                     # ai_memory - Qdrant, PAI memory
│   └── agents/                     # ai_agents - Base agent framework
│
├── services/
│   ├── api/                        # FastAPI backend
...
/home/wyld-api/                     # API deployment (symlinks)
├── current -> /home/wyld-core      # Symlink to repo
...
/home/wyld-data/                    # All runtime data (gitignored)
├── logs/
│   ├── agents/
│   ├── api/
│   ├── web/
...
/home/wyld-monitor/                 # Monitoring (symlinks to repo configs)
├── prometheus -> /home/wyld-core/infrastructure/monitoring/prometheus
├── grafana -> /home/wyld-core/infrastructure/monitoring/grafana
└── loki -> /home/wyld-core/infrastructure/monitoring/loki
```

## Permission Model

### Users & Groups
All wyld-* users are members of the `wyld` group (GID 2000):
- wyld-core (1014): sudo, docker - runs agents, owns source
- wyld-api (1015): docker - API service
- wyld-web (1016): docker, www-data - web frontend
- wyld-data (1017): docker - data storage
- wyld-monitor (1018): docker - monitoring

### Access Rules
- Source code: readable by wyld group (750)
- Secrets (.env): readable ONLY by wyld-core (600)
- Logs/backups: writable by wyld group (770)
- Scripts: executable by wyld-core (750)

### For AI Agents (infra-agent)
Running as wyld-core, you can:
- Read all /home/wyld-*/ directories
- Write to /home/wyld-data/logs/ and /home/wyld-data/backups/
- Execute scripts in /home/wyld-core/infrastructure/scripts/
- Access Docker via docker group membership
- Read secrets from /home/wyld-core/.env
- Use sudo for system administration (sparingly)
...
```bash
# Start dev environment (uses .env for API keys)
make dev

# Start agents (as wyld-core)
sudo -u wyld-core make agents
...
### Adding a New Agent
1. Copy `agents/_template/` to `agents/your_agent/`
2. Update `config/agents.yaml` with agent definition
3. Run `make install` to install the new package
4. Test with `make test`
5. Add to `infrastructure/scripts/start-agents.sh`
